...
def image_draw(epd_width, epd_height, config_file_name="config.txt"):
    ...
    if fetchWeather:
        ...
        r = requests.get("http://api.positionstack.com/v1/forward?access_key={}&query={}&limit=1".format(positionstack_api_key, query))
        geolocation = r.json()
        ...
        r = requests.get(
            "https://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&appid={}&units={}".format(weather_lat, weather_long, owm_api_key, weather_units))
        ...
        draw.text((weather_topleftx + 8, epd_height - 15), "{}".format(geolocation["data"][0]["label"]), font=textfont12)
        draw.text((epd_width - 110, epd_height - 15), "Updated {}:{}".format(now.hour, now.minute), font=textfont12)
    return Himage
